...
namespace FactoryModule
{
    public partial class UnitType : Form
    {
        ...
        public void DataGrid()
        {
            ...
            cmd.CommandText = "SELECT UnitID as 'Unit ID', UnitName as 'Unit Name' FROM FactoryUnit";
            cmd.ExecuteNonQuery();
        ...
        private int AlreadyHas(string unitname)
        {
            ...
            string query = "SELECT * FROM FactoryUnit WHERE UnitName = '" + unitname + "'";
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    string conStr = ConfigurationManager.ConnectionStrings["PosConString"].ToString();
                    SqlConnection connection = new SqlConnection(conStr);
                    string query = "INSERT INTO FactoryUnit(UnitName) VALUES('" + unit_name + "')";
                    SqlCommand command = new SqlCommand(query, connection);
                    connection.Open();
        ...
        private void buttonUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                string new_unit_name = textBoxUnitName.Text;
                if (new_unit_name == "")
                {
                    MessageBox.Show("Please Fill the text box.");
                }
                ...
                else
                {
                    string conStr = ConfigurationManager.ConnectionStrings["PosConString"].ToString();
                    SqlConnection connection = new SqlConnection(conStr);
                    string query = "UPDATE FactoryUnit SET UnitName = '" + new_unit_name + "' WHERE UnitID ='" + textBoxUnitID.Text+ "'";
                    SqlCommand command = new SqlCommand(query, connection);
                    connection.Open();
                    int rowEffict = command.ExecuteNonQuery();
                    connection.Close();
                    if (rowEffict > 0)
                    {
                        textBoxUnitName.Text = textBoxUnitName.Text =  string.Empty;
                        DataGrid();
        ...
        private void dataGridViewViewUnit_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            textBoxUnitName.Text = dataGridViewViewUnit.SelectedRows[0].Cells[1].Value.ToString();
        }
